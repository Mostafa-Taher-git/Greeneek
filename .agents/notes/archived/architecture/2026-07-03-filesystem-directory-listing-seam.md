...
Add `FileSystem.listDir(target, signal?)` to `@greeneek/gnk-fs`.
...
**Keep directory enumeration in each consumer.** Rejected. That would bind product packages such as `gnk-skill` to Node/local filesystem behavior and bypass policy/remote/sandboxed backends.
