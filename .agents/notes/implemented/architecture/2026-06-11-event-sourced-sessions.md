...
- Persistence stays a plugin concern; the in-memory store ships in gnk-session.
...
- Derivation cost grows with log length — compaction (gnk-compaction) is the intended mitigation, not log mutation.
