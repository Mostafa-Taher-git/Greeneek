...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `gnk-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `gnk-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `gnk-timeout` (`idleWatchdog`) |
| Clear the timer | `gnk-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `gnk-timeout` (`timeoutOf`) |
