...
`@greeneek/gnk-tool-todo` declares `TodoItem` and merges `todo/write` into `@greeneek/gnk-session/types` from its type-only outlet. The package root and `/client` entrypoint re-export `TodoItem`, so host and browser consumers share one declaration without loading the todo plugin.
