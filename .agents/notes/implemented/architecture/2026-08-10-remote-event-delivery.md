...
### Consumer contract (`gnk-typert-protocol`)
...
```ts ignore-check
import type { Events } from '@greeneek/cordis'
...
JSON safety remains a runtime concern. Before queueing, the API Remotes Host source checks every argument with `gnk-session`'s `isJsonValue` and fails loudly when one is invalid, because this is an allowlist composition error rather than untrusted input.
...
| Root `tsconfig.base.json` | Adds source-plane `paths` entries for `gnk-settings/types`, `gnk-credentials/types`, and `gnk-api-remotes/types` |
| `gnk-commands` / `gnk-settings` / `gnk-credentials` | Moves each `interface Events` member to the owner's Client-safe `./types`; settings and credentials add that export, move brands and pure types with it, retain constructors in index, and include `lib/types/**/*.js` in published files |
| `gnk-session` | Exposes `isJsonValue` for validation of every event argument by the API Remotes Host source |
