...
- The served rendering of the manifest global changed from `window.__GNK_BOOT__ =` to `globalThis["__GNK_BOOT__"] =`; no committed snapshot expectation carries that text, so none needed re-recording.
