# Agent Note: One gnk launcher for application profiles
...
Greeneek Harness application processes need one owner for composition, plugin resolution, environment discovery, shutdown, and user customization. A dedicated app bin with a complete `cordis.yml` creates a second lifecycle beside profile launch: plugins installed into a profile do not reach it, behavior drifts from `gnk-base`, and SDK callers learn arbitrary process argv instead of the product's composition model.
...
Every supported Node application starts through the `gnk` CLI and one named profile. The shipped application commands are `gnk web`, `gnk --profile headless`, `gnk --profile sdk`, `gnk --profile sdk-minimal`, and `gnk --profile acp`; `gnk web` is the deliberate convenience alias for `--profile web`, not another application entry.
...
Custom profiles default to `live`. A startup profile still applies its bundle, profile, home-level, and invocation `--patch` layers, but it does not watch them after boot. `gnk-base` inserts the module-HMR row disabled; a profile with a tested source-module reload lifecycle must enable it explicitly. None of the shipped profiles enable server module HMR: `patchReload: live` uses the launcher's config-only watcher while the startup profiles install no watcher. SDK and ACP cannot safely replace their server, agents, persistence, or tool registry inside one owned stdio connection.

The shipped protocol profiles reserve stdout for protocol frames, expose help without starting transport, and route stdin EOF and signals through bounded root disposal. ACP remains automation-only. The SDK JSON-RPC methods, notification fields, and `initialize.serverInfo.name` remain stable. Full-profile model-visible tool and persistence defaults come from `gnk-base`; `sdk-minimal` owns its explicit defaults. Runnable snapshots own the assembled application outputs.

### TypeScript SDK customization

`@greeneek/gnk-sdk-client` depends on the same-version `@greeneek/gnk` package, resolves its installed CLI module, runs it through the current Node executable, and selects `sdk` by default. Both client layers expose `gnkBin`, `profile`, ordered `patches`, `gnkHome`, process cwd, environment, and timeouts; arbitrary command/argv launch remains an internal fake-runtime adapter.

SDK users customize plugins through profiles. `gnk plugin --profile <name> ...` manages persistent dependencies and bundle order, the profile's `cordis.patch.yml` owns persistent row changes, and launch `patches` supply ordered ephemeral overrides. A custom profile must retain `@greeneek/gnk-sdk-app` or another SDK server row. Relative CLI-module, patch, explicit home, and process-cwd paths become absolute before spawn, and initialization has a finite bound whose diagnostic names the selected profile.

Direct SDK use follows normal Harness-home resolution: explicit `gnkHome`, inherited `GNK_HOME`, then `~/.gnk`. `subagent-gnk-sdk` instead requires an explicit absolute home, so a nested runtime cannot discover a person's profiles, installed plugins, credentials, or sessions through the operating-system home. GNK-specific ACP child examples also pass an isolated home; the ACP backend itself remains generic for non-GNK agents.

### Python runtime

The Python runtime wheel packages the ordinary `@greeneek/gnk` CLI from `node_modules/@greeneek/gnk/lib/bin.js` through the private `gnk-python-runtime-closure` deploy manifest. The Python client selects `gnk --profile sdk` by default, ordered patch files, and an explicit Harness home; the runnable example under `python/sdk/examples` selects `sdk-minimal`. The installed `gnk` console command exposes the same profile grammar and the separately packaged `web` application.

The executable family is `greeneek-harness-sdk-runtime-<platform>-<arch>`. The SDK wire, wheel and import distribution names, sidecar names, and wire identity `greeneek-harness-sdk-runtime` remain stable. The SDK package family is `@greeneek/gnk-sdk-client`, `@greeneek/gnk-sdk-protocol`, and `@greeneek/gnk-sdk-jsonrpc-server`; `@greeneek/gnk-acp` remains the ACP protocol plugin. There is no Python-specific Node application, checked-in complete config, compatibility package, forwarding executable, fallback parser, or SDK/ACP launcher alias. The [Python profile-runtime decision](2026-08-23-python-sdk-gnk-profile-runtime.md) owns this launch, and the [Windows x64 runtime decision](2026-08-23-python-sdk-windows-x64-runtime.md) owns the fourth carrier.

### Enforcement

`verify-application-entrypoints` scans application/package manifests, executable sources, and root demo scripts. The allowlist classifies the `gnk` product bin, vendor-excluded scope, the private WebWorker build tool, and test support. An unclassified shebang, a new package bin, or a demo wrapper that bypasses `apps/cli/src/bin.ts` fails hygiene and the primary/static CI aggregates.
...
**Resolve `gnk` only from `PATH`.** Rejected: ordinary Node processes do not reliably inherit a project-local `.bin` path. A same-version package dependency provides a deterministic runtime.

**Enable module HMR in `gnk-base` and make unsafe profiles disable it.** Rejected: the shared base also underlies custom profiles, so an enabled default makes every new application remember to opt out of source-module replacement. A disabled base makes module HMR an explicit profile capability while leaving `patchReload: live` config watching available.
...
- Bundle configuration tests pin module HMR disabled in `gnk-base` and absent from shipped mode overrides; the custom live-profile e2e pins config reload through the launcher's watch-only fallback.
...
- Keyless ACP and SDK snapshots boot real `gnk` profiles and pin protocol output plus persisted logs; the nested SDK composition boots a second real profile runtime.
- The real-API workflow caps file parallelism at four because one profile e2e file can own several complete `gnk` subprocess trees; workflow tests pin that resource bound.
- The Python suite exercises exe and node carriers; packaged-runtime scenarios, native macOS executable construction, both wheels, and clean-wheel default/MCP smokes pin the `greeneek-harness-sdk-runtime-*` artifacts and profile launch.
...
- A user changes an SDK application's plugin composition through a named profile and ordered patches, using the same installation and resolution model as every other gnk application.
...
- Adding `@greeneek/gnk` increases the TypeScript client's install size in exchange for a deterministic same-version runtime.
...
- Python packages the ordinary `gnk` profile launcher while retaining a closed native runtime and no system-Node requirement for wheel users.
