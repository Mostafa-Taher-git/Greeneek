...
The Client connection stops re-exporting `ToolCallView` and `ToolResultView` from `gnk-tools/presentation` for Session consumers.
...
- Session history does not reference `gnk-tools/presentation`, `ctx.tools`, `presenterScopeFor`, or `backscanArgs`.
...
Changes to this decision use `gnk-pre-push-checks` to select commands for the final diff. Required evidence includes:
...
- `GNK_SNAPSHOT=replay pnpm run test:web`, without refreshing existing presentation goldens;
