...
| Package | Role |
|---|---|
| `@greeneek/gnk-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@greeneek/gnk-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@greeneek/gnk-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@greeneek/gnk-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@greeneek/gnk-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@greeneek/gnk-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@greeneek/gnk-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
