...
The first Windows-native foundation shipped `gnk-tool-pwsh` as a deliberately minimal profile — foreground only (a fresh process per call; no persistent PTY session), no managed-environment parity beyond three hardcoded `GNK_*` keys, and a marker story ("always `[exit code: N]`") that diverged from the bash tool's rendering without being declared. The model-visible contract drifted from the implementation: the description promised spill-path reporting the renderer never performed, the README claimed exports that did not exist and rendering the tool did not do, and the tool's own tests pinned the lossy behavior. The minimal profile also left the `GNK_*` contributor seam duplicated-by-absence: plugins contributing environment facts to `ctx.shellEnv` had no effect on pwsh calls.

## Decision

`gnk-tool-pwsh` now mirrors `gnk-tool-bash` call-for-call, and its model-visible text describes exactly that behavior:
...
- `@greeneek/gnk-shell-env` is a new shipped package; `gnk-tool-bash`'s `gnkHome` config moved there, so compositions mounting the shell tools must also mount `shell-env` (the spine bundles do).
