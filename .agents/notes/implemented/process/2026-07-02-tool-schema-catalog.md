...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `gnk-tool-bash` (`bash`), `gnk-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `gnk-tool-subagent` (`subagent`). Example-only tools are excluded.
