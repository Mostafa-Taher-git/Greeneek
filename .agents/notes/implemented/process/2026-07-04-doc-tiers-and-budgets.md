...
- **One product onboarding path.** The root README owns the recommended package-run path, the source-run alternative, and compact `gnk plugin --profile` usage. The published user guide starts with tasks inside the running Web UI, then links to distinct tutorials or reference owners for other interfaces, plugin development, and advanced configuration instead of repeating Web startup.
...
- **A thin workflow skill, contracts in docs.** [.agents/skills/gnk-doc](../../../skills/gnk-doc/SKILL.md) carries the placement, audit, budget, and website workflow and defers to the standard as its source of truth, the same split as [gnk-translate-docs](../../../skills/gnk-translate-docs/SKILL.md) over the i18n contract.
