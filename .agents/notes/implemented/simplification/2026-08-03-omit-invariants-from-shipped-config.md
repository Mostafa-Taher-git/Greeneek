# Agent Note: Omit runtime invariants from shipped gnk config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@greeneek/gnk-*/invariant` entry.
...
- Ordinary `gnk` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
