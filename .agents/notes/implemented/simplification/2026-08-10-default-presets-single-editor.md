...
The `standard`, `code`, and `cordis` preset configurations mount `gnk-tool-fs` and `gnk-tool-fs-search`, but do not mount `gnk-tool-str-replace-editor`. PTC mode therefore omits `str_replace_editor` from both its registry and generated SDK. The `minimal` preset continues to mount `gnk-tool-str-replace-editor`, and deployments or user-authored presets may still mount the plugin explicitly.
