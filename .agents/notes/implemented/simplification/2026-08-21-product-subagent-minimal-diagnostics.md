...
| Exit code and signal | `gnk-subprocess` handle | Displayed independently when observed, without inferring missing values |
...
| Diagnostic delivery and byte limit | `gnk-subagent` and its foreground or Job consumers | One bounded optional string remains separate from assistant output |
