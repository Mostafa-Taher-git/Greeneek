...
The `demo:acp` and `demo:cordis` scripts are absent. ACP starts through `gnk --profile acp`; the Cordis guide starts `gnk web` with its explicit overlay. `demo:ptc` remains as a thin wrapper over `gnk --profile headless` with `GNK_TOOLS_MODE=ptc`.
