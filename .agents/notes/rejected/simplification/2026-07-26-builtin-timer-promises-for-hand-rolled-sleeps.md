...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`gnk-llm-mock-server` `pause()`, `gnk-lsp-stdio`, `gnk-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
