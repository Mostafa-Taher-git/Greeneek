...
- **`eventsource-parser` for apiproxy's `readSse`**: only ~15 lines of framing are deletable, both wire ends are in-repo so spec conformance is moot, and it would add a dep to a browser-safe package. (Contrast with the [archived llm-greeneek dependency decision](../../archived/simplification/2026-07-26-eventsource-parser-for-greeneek-sse.md), where a real provider sits across the wire.)
