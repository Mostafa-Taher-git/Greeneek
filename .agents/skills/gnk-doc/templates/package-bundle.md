...
Use this template for a package whose manifest declares `gnk.bundle.patch` — an installable profile layer: `packages/bundle/*`, `gnk-subagent-codex`, `gnk-subagent-claude-code`. The `bundle/base` README pair is the worked example.
...
```yaml
---
description: "What the bundle layer adds to a gnk --profile surface, for users composing or customizing a profile."
kind: "package-bundle"
---
```
...
```text
gnk plugin --profile <name> add @greeneek/gnk-<name>
gnk plugin --profile <name> remove @greeneek/gnk-<name>
```
