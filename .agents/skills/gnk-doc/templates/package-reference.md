...
The generated [configuration catalog](../../../docs/config-catalog.md#greeneekgnk-<name>) is the exhaustive source for every accepted field.
...
- **Installation guidance.** A plugin package mounts through `cordis.yml` rows. Only a package declaring `gnk.bundle.patch` installs as a profile layer via `gnk plugin --profile <name> add <package>` — if this package lacks that declaration, say how it mounts in a composition, never `gnk plugin add`.
