 ...
 * the REAL shipped bundle layers (gnk-base + gnk-web-app resolved from the
...
import { evaluate } from '@greeneek/cordis-plugin-loader'
import { SHIPPED_PRESET_ROOT } from '@greeneek/gnk-agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@greeneek/gnk-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL gnk-base/gnk-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'gnk-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@greeneek/gnk-base', '@greeneek/gnk-web-app'])
    const profile = loadProfile('gnk', 'web', anchor, home)
    ...
    for (const name of ['@greeneek/gnk-pwsh-sandbox', '@greeneek/gnk-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'gnk-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@greeneek/gnk-base'])
    const profile = loadProfile('gnk', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
