...
the deliberate composition divergences from `gnk web` — are documented in
...
## Do not import `@greeneek/gnk-client-*` here
...
`@greeneek/gnk-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@greeneek/gnk-client-web` and the boot-manifest
type from `@greeneek/gnk-client-modules/client`: booting the real shell is what
