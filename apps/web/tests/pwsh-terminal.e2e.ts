...
import { resolvePwshPath } from '@greeneek/gnk-pwsh-local'
import {
...
// The overlay swaps the shipped bash executor for @greeneek/gnk-pwsh-local;
...
describe.skipIf(MODE === 'record' || !HAS_PWSH)('web e2e: pwsh calls use the bash terminal-card layout', () => {
  ...
  it('renders the seeded pwsh call as a terminal card with the parsed exit pill', async () => {
    ...
    await result.click()
    await page.getByRole('button', { name: 'Session options', exact: true }).waitFor({ timeout: 15_000 })
