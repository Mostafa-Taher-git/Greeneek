...
- Optional: a Greeneek API key for the Web, headless, and ACP automation demos and real-API e2e tests.
...
The install also configures worktree-local Lefthook hooks and the `gnk-translation-pairing` Git merge driver through `scripts/install-lefthook.mjs`. The [worktree-local hooks Agent Note](../.agents/notes/implemented/process/2026-07-27-worktree-local-lefthook.md) owns the hook-path safety contract; the [automatic pairing merges Agent Note](../.agents/notes/implemented/process/2026-08-08-automatic-translation-pairing-merges.md) owns the merge driver.
...
tsdown --env.GNK_BUILD_FACE host
tsc -b tsconfig.client.json
tsdown --env.GNK_BUILD_FACE client
pnpm run build:web
```

Both tsdown passes use the same complete workspace match. They neither scan build artifacts to discover Client packages nor maintain a Host/Client package filter list. Package-local tsdown configs select entries for the current phase through `GNK_BUILD_FACE`: an ordinary Client plugin produces both its Node loader and browser bundle during the Client phase; `api-remotes` uses `hostPhase: true` to produce its Host entry early and only its browser bundle during the Client phase. Tsdown consumes only the JavaScript emitted to `lib/types` by the preceding tsc phase.
...
The real Greeneek adapter and key-backed agent demos read credentials from the environment or from a gitignored `.env` at the repo root:

```sh
GREENEEK_API_KEY=sk-...
GREENEEK_BASE_URL=https://... # optional
```

`GREENEEK_BASE_URL` is optional and defaults to the public API. Never commit real credentials. The real-API e2e suites self-skip when `GREENEEK_API_KEY` is not set.
...
The one-shot Headless coding agent needs `GREENEEK_API_KEY` in the environment or repo-root `.env`:

```sh
pnpm gnk --profile headless "summarize this workspace"
```
