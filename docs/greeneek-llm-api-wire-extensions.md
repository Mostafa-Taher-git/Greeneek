...
|---|---|---|
| HTTP field names | Lowercase kebab-case; HTTP matching remains case-insensitive | `user-agent`, `x-greeneek-harness-session-id` |
| Greeneek request-body extension fields | Snake case with the reserved `gnk_` prefix | `gnk_plugin_packages`, `gnk_session_log` |
| GNK-owned nested JSON members | Camel case | `afterSeq`, `throughSeq`, `sessionId` |
| Tagged values | Kebab-case strings; durable events use `domain/action` | `session-log-greeneek/delivery-accepted` |
...
The [`GreeneekLlmApiExtensionRegistry`](../packages/llm/greeneek-llm-api-extensions/README.md) reserves one provider per top-level extension name. Empty or whitespace-padded names, duplicate registrations, and collisions with the base Greeneek request fail before HTTP dispatch.

## Request headers

| Header | Presence | Value |
|---|---|---|
| `user-agent` | Every provider HTTP request, including Files API operations | Application identity in `product/version (+url)` form; the default product is `greeneek-harness` |
| `x-greeneek-harness-user-id` | Every authorized chat-completion request | The stable anonymous UUID for the resolved Harness home |
| `x-greeneek-harness-session-id` | Chat-completion requests carrying a Session id | The exact request `sessionId` string |
| `x-greeneek-harness-compact` | Chat-completion requests whose purpose is `compaction` | The literal string `1` |

Credential failure happens before anonymous-user-id resolution, so an unauthorized request neither sends these headers nor creates the identity file. A direct request without a Session omits `x-greeneek-harness-session-id`. Session-title requests have no additional purpose header; the ordinary Session-id rule still applies when one carries a `sessionId`.
...
[`@greeneek/gnk-plugin-package-inventory-greeneek`](../packages/llm/plugin-package-inventory-greeneek/README.md) contributes the complete active Loader-backed plugin package inventory. The field is enabled by default.
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `1` | Schema version for `gnk_plugin_packages` |
...
An enabled inventory with no qualifying entries sends `packages: []`; disabling the contributor omits the entire `gnk_plugin_packages` field. Package identities are provider metadata and never enter model input.

## `gnk_session_log`

[`@greeneek/gnk-session-log-greeneek`](../packages/session/session-log-greeneek/README.md) contributes one contiguous suffix of the canonical Session log. The field is disabled by default. When enabled, it applies to a request with a live Session and at least one event; a direct request, a stale Session id, or an empty log omits the field.
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `1` | Schema version for `gnk_session_log` |
...
The `session` member is the exact `Session.header`, not a complete runtime Session. The outer `gnk_session_log.version` selects this extension schema, while `session.version` selects the canonical on-disk Session format; the two version values evolve independently.
...
The request headers expose the Harness application version, one anonymous Harness-home identity, and an optional Session identity. `gnk_plugin_packages` exposes active npm package names and versions. When enabled, `gnk_session_log` may expose the Session working directory, system-prompt snapshots, user and assistant content, raw assistant chunks, tool arguments and results, compaction summaries, feedback, and plugin-owned events. Adapter API keys are not Session events and therefore do not enter the field. A gateway selected through `baseURL` receives the same values as the official endpoint.
