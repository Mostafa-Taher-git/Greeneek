...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `GNK_*` environment vocabulary |
...
| [client-modules.md](client-modules.md) | the web plugin table: `gnk.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
...
| [desktop.md](desktop.md) | the installable Electron shell: `gnk web` service hosting, single-instance lock, tray, splash, security policy, safe mode, and consent-based self-updates |
