...
```ts type-equiv
/** The composed client entry graph the host injects as `window.__GNK_BOOT__`. */
...
The web plugin table service: incremental `gnk.client` scan + wire composition + bundle route + index injection rows. Construction runs the activation scan synchronously — a malformed declaration or missing bundle among the already-loaded entries aggregates into one loud throw (FAILED fiber; the boot activation audit reports it).

```ts cordis-catalog
/**
 * Current composed entry graph (stable object between changes).
 * @returns the graph served as `window.__GNK_BOOT__`.
 */
graph(): WebBootGraph
