...
`gnk-fs-observation-policy` is optional. Without it, the `FileSystem` Service Definition, a provider, and the `gnk-tool-fs` Consumer form the complete, unconstrained filesystem seam: `write` unconditionally creates or overwrites, and `edit` unconditionally replaces literal text. The policy plugin changes these operations by deciding the `fs/*` waterfalls. Removing it does not break the tool because the tool calls `ctx.fs` and dispatches events; it does not call policy methods. A deployment that loads `gnk-tool-fs` is expected to also load `gnk-fs-observation-policy` so the default behavior is read-before-write/edit.
...
`gnk-fs` owns three events the tool dispatches and the policy plugin listens for, so the emitter (`gnk-tool-fs`) and the listener (`gnk-fs-observation-policy`) share a vocabulary without the emitter depending on the policy plugin. They carry only `gnk-fs` vocabulary plus an opaque `object` actor — no model-facing concepts and no agent/session owner structure.
 ...
 * `FsObservationActor` without importing `gnk-tools`, `gnk-agent`, or `gnk-session`.
 *
