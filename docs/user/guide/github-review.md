...
This opt-in overlay adds a signed GitHub endpoint to `gnk web`. When a pull request in the configured repository changes from draft to ready for review, the rule creates a titled root Session under the repository's Web Workspace and starts a read-only review prompt.

## Prerequisites

- A local checkout that GNK may register as a Web Workspace.
- A high-entropy GitHub webhook secret available through the `GNK_GITHUB_WEBHOOK_SECRET` credential reference.
...
The overlay defaults the Workspace to the launch directory and the listener to `127.0.0.1:3081`. Override them with `GNK_GITHUB_REVIEW_WORKSPACE` and `GNK_GITHUB_WEBHOOK_PORT`.
...
```sh
export GNK_GITHUB_WEBHOOK_SECRET="$(openssl rand -hex 32)"
printf '%s\n' "$GNK_GITHUB_WEBHOOK_SECRET"
```

From a development checkout:

```sh
export GNK_GITHUB_REVIEW_WORKSPACE=/path/to/greeneek-harness
pnpm gnk web --patch apps/cli/config/examples/github-review/cordis.yml
```

An installed GNK uses the same overlay through an absolute path:

```sh
gnk web --patch /absolute/path/to/github-review/cordis.yml
```

For a permanent profile, place `github-ready-review-rule.mjs` beside `$GNK_HOME/profiles/web/cordis.patch.yml`, append the rows from `cordis.yml` to that patch, and start with `gnk web`. The shipped CLI already contains both webhook packages; the overlay alone activates them.
...
Secret:       GNK_GITHUB_WEBHOOK_SECRET value
...
The rule accepts only source `primary-github`, repository `greeneek-harness/greeneek-harness`, event `pull_request`, and action `ready_for_review`. It passes the exact head SHA plus selected PR fields to the review prompt, labeling the JSON as untrusted metadata and forbidding file, branch, PR, or GitHub mutation.
...
```js
const workspacePath = {
  'greeneek-harness/greeneek-harness': '/path/to/greeneek-harness',
  'greeneek-harness/gnk-sdk': '/path/to/gnk-sdk',
