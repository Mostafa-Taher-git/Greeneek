...
import type { WorkspaceSnapshot, WorkspaceView } from '@greeneek/gnk-api-workspace-controller/client'
...
import { en as commonEn } from '@greeneek/gnk-client-locale/src/locales/en.ts'
...
describe('ConversationRoot resident composer', () => {
  ...
  it('publishes the column width as a px variable for the shared width axis', () => {
    ...
    act(() => { fireResize(root) })
    expect(root.style.getPropertyValue('--gnk-conversation-column-width')).toBe('1200px')
    // No dragged preference: the user-width override stays absent so the
    // adaptive clamp term applies.
    expect(root.style.getPropertyValue('--gnk-chat-user-width')).toBe('')
  })
  ...
  it('drag → persist → window clamp round-trip on a width handle', () => {
    ...
    try {
      ...
      expect(root.style.getPropertyValue('--gnk-chat-user-width')).toBe('970px')
      expect(localStorage.getItem('gnk.conversation.contentWidth')).toBe('970')
      ...
      act(() => { fireResize(root) })
      expect(root.style.getPropertyValue('--gnk-chat-user-width')).toBe('724px')
      expect(localStorage.getItem('gnk.conversation.contentWidth')).toBe('970')
