...
import * as primitives from '@greeneek/gnk-client-ui-primitives'
import {
...
describe('FishLogo', () => {
  it('renders the Greeneek logo image at the requested size', () => {
    const { container } = render(<primitives.FishLogo />)
    const img = container.querySelector('img')!
    expect(img.getAttribute('src')).toBe('/assets/logo-mark.png')
...
describe('BrandWordmark', () => {
  it('renders the name as text and controls the leading mark independently', () => {
    const view = render(<primitives.BrandWordmark />)
    expect(view.container.textContent).toBe('Greeneek')
