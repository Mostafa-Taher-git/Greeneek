...
import type { WorkspaceId, WorkspaceView } from '@greeneek/gnk-api-workspace-controller/client'
...
export function WorkspaceBrowser({
  ...
  // Session delete dialog mirrors the Workspace one above, with one extra
  // wait dimension: the row leaves the Workspace accounts through the follow
  // feed and the Session list through the api-session/removed relay, so the
  // committed id clears only once it is absent from both projections.
  ...
  useEffect(() => {
    if (sessionDeleteCommittedId === null) return
    if (sessionIds.includes(sessionDeleteCommittedId)) return
    if (workspaces.some(workspace => workspace.sessionIds.includes(sessionDeleteCommittedId))) return
    setSessionDeleting(false)
    setSessionDeleteCommittedId(null)
    setSessionDeleteTarget(null)
  }, [sessionDeleteCommittedId, sessionIds, workspaces])
