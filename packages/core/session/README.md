...
Mount `gnk-session` wherever a session must exist. It creates and holds event-sourced `Session` instances in memory; durable storage is layered on by a persistence plugin that subscribes to the `session/event` feed.
...
| [`gnk-util-values`](../../util/values/README.md) | Shared lossless JSON validation and detached snapshots |
