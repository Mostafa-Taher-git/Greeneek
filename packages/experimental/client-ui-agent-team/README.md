...
The Client export mounts the generated `ctx.remote.agentTeams` contribution from [`@greeneek/gnk-experimental-agent-team/remote`](../agent-team/README.md), then registers its locale dictionaries and one conversation-header slot through Cordis effects. Disposing the plugin fiber removes both registrations.
