...
Indirectly, through PTC mode in `gnk-tools` when an explicit source-checkout composition mounts this provider; it renders the program's completion value or failure into a retained `run_code` result, and no shipped profile mounts this private package.
...
- **The truncation-marker text and the tempdir prefix keep the pre-rename short names** — the marker `[gnk-code-runtime-python] log capture truncated at <N> bytes` and the `gnk-code-runtime-python-` tempdir prefix are byte-anchored by tests and are independent of the npm package name; promotion (dropping the `experimental-` prefix) does not rename them.
