...
import { resolveRetryPolicy, RetryPolicySchema } from '@greeneek/gnk-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@greeneek/gnk-llm'
import {
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with gnk-llm-retry',
    )
  }
}
