...
# @greeneek/gnk-plugin-package-inventory-greeneek
...
The version-1 `gnk_plugin_packages` field contains only `{ name, version }` pairs. Disabled, pending, failed, disposed, unloading, structural `cordis:` rows, ordinary dependencies, loose files without an owning package identity, programmatically mounted child fibers, and in-memory dynamic plugins are excluded.
...
Nothing. `gnk_plugin_packages` is provider metadata outside the model's messages, system prompt, and tool schemas.
