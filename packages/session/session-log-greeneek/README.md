...
Direct requests without a live Session omit `gnk_session_log`. Normal agent, compaction, and session-title calls carry their live Session id.
...
Nothing. `gnk_session_log` is a sibling of the Greeneek request's model-input fields and is not inserted into `messages`, the system prompt, or tool schemas.
