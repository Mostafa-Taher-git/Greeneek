...
- id: sessionTelemetry-otel
  name: '@greeneek/gnk-session-telemetry-otel'
  config:
...
The generated [configuration catalog](../../../docs/config-catalog.md#greeneekgnk-session-telemetry-otel) is the exhaustive source for every accepted field. Upload authorization is positive and fail-closed: an unknown direct-construction mode fails before transport configuration is read, only `FULL` accepts direct `ctx.sessionTelemetry.emit()` calls, and `FEEDBACK_ONLY` treats only the exact `feedback/record` object already stored in the canonical log as consent.
...
The backend is a thin adapter over the OTel JS SDK: it owns capture mode, resource identity, and an outer shutdown deadline, and passes everything else through verbatim. Two instrumentation scopes separate record channels — ledger records on `@greeneek/gnk-session-telemetry-otel`, operational records on `@greeneek/gnk-session-telemetry-otel/ops` — so receivers can alert on ops without summing them. Resource identity carries `service.name`/`service.version` from `gnk-llm`'s `APP_IDENTITY` plus the package's anonymous `user.id` (from `$GNK_HOME/.anonymous-user-id`), once per export batch rather than per record.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#greeneekgnk-session-telemetry-otel) — every accepted config field and its source declaration.
