...
Use `gnk-skill` when agents should load skills from more than one source through one interface, or when the source of skills is not the local filesystem. Avoid it when a composition needs no skill loading at all — the plugin adds a service and a per-lookup discovery cost. The shipped local provider (`gnk-skill-filesystem`) and the model-facing consumer (`gnk-tool-skill`) are separate packages; mount them alongside when the deployment wants local skills and model access.
...
The generated [configuration catalog](../../../docs/config-catalog.md#greeneekgnk-skill) is the exhaustive source for every accepted field.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#greeneekgnk-skill) — every config field and its source declaration.
...
Indirectly, through `gnk-tool-skill`, which renders provider summaries into durable initial or replacement catalog messages and loaded instruction bodies into retained tool results.
