...
import type { SaveTextSpill } from '@greeneek/gnk-spill'
import LocalSpillStore, {
  ...
  sessionDir,
  sweepSpillRoots,
} from '@greeneek/gnk-spill-local'
import type { SweepRoot } from '@greeneek/gnk-spill-local'
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'gnk-spill-test-'))
})
afterEach(() => {
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'gnk-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('keeps a file exactly at the boundary (only strictly-older expires)', async () => {
    ...
    // Whole-second mtime resolution: round UP so float seconds can never land
    // the instant a hair below the cutoff and flake the strictly-older rule.
    utimesSync(boundary, Math.ceil(cutoffMs / 1000), Math.ceil(cutoffMs / 1000))
  ...
  it('prunes empty active session directories after deleting expired files', async () => {
    ...
    mkdirSync(emptied, { recursive: true, mode: 0o755 })
    mkdirSync(kept, { recursive: true, mode: 0o755 })
  ...
  it('skips a symlink INSIDE a session dir and non-session siblings', async () => {
    ...
    const unrelated = join(root, 'not-a-session'); mkdirSync(unrelated, 0o755)
  ...
  it('does NOT follow a symlinked session directory (no deletion in the target)', async () => {
    ...
    const victimDir = join(root, 'victim'); mkdirSync(victimDir, { recursive: true, mode: 0o755 })
  ...
  it('sweeps only exact session-<12hex> names, not lookalikes', async () => {
    ...
    const backup = join(root, 'session-backup'); mkdirSync(backup, { recursive: true, mode: 0o755 })
    const backupOld = join(backup, 'old.txt'); writeAged(backupOld, 'x', 40)
    const shortHex = join(root, `session-${'a'.repeat(11)}`); mkdirSync(shortHex, { recursive: true, mode: 0o755 })
    ...
    const real = sessionDir(root, 'sess-1'); mkdirSync(real, { recursive: true, mode: 0o755 })
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'gnk-spill-'))
    const priorDir = sessionDir(prior, 'old-sess'); mkdirSync(priorDir, { recursive: true, mode: 0o755 })
    writeAged(join(priorDir, 'old.txt'), 'x', 40)
    const activeDir = sessionDir(root, 'sess-1'); mkdirSync(activeDir, { recursive: true, mode: 0o755 })
  ...
  it('does NOT prune a discovered root that still holds a fresh file', async () => {
    const prior = mkdtempSync(join(tmpdir(), 'gnk-spill-'))
    const priorDir = sessionDir(prior, 'sess'); mkdirSync(priorDir, { recursive: true, mode: 0o755 })
...
describe('discoverDefaultRoots', () => {
  it('returns only real gnk-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'gnk-disc-'))
    try {
      // A real backend-shaped root (gnk-spill-<6>) via mkdtemp — the only match.
      const realRoot = mkdtempSync(join(base, DEFAULT_ROOT_PREFIX))
      mkdirSync(join(base, 'unrelated-dir'), 0o755)
