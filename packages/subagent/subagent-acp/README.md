...
The generated [configuration catalog](../../../docs/config-catalog.md#greeneekgnk-subagent-acp) is the exhaustive source for every accepted field and its JSDoc.

A Greeneek Harness child uses the product launcher and an explicit absolute `GNK_HOME`. The isolated home prevents a nested runtime from discovering the launching person's profiles or credentials; the generic ACP provider does not impose this requirement on non-GNK agents.

```yaml
- id: subagent-acp
  name: '@greeneek/gnk-subagent-acp'
  config:
    providerName: acp
    command: gnk
    ...
    permission: reject
    env:
      GNK_HOME: /absolute/path/to/isolated-child-home
...
- [gnk-subprocess seam](../../subprocess/subprocess/README.md) — the process-spawn and teardown machinery behind each run.
- [Generated configuration catalog](../../../docs/config-catalog.md#greeneekgnk-subagent-acp) — every accepted config field and its source declaration.
