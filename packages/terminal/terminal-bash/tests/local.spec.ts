...
import type { TerminalSendOperation } from '@greeneek/gnk-terminal'
...
import * as ptyLocal from '@greeneek/gnk-terminal-bash'
...
async function harness(
  ...
  const root = mkdtempSync(join(tmpdir(), 'gnk-pty-local-'))
  roots.push(root)
...
describe.skipIf(process.platform === 'win32')('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    ...
    try {
      ...
      if (previous === undefined) delete process.env.GNK_TEST_SECRET
      else process.env.GNK_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('restores the controlled prompt after an in-shell PS1 override', async () => {
    ...
    expect(result.viewport).toContain('healed=[gnk> ]')
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' gnk "${pidFile}" & disown`,
        submit: true,
      })
