...
```ts
import { brandString, type Branded } from '@greeneek/gnk-brand'
...
```ts
import { brandNumber, type BrandedNumber } from '@greeneek/gnk-brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `gnk-llm`, the shared agent/session `SessionId` in `gnk-session`, `JobId` in `gnk-jobs`, and `SessionSeq` versus `SessionLogOffset` in `gnk-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `gnk-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
