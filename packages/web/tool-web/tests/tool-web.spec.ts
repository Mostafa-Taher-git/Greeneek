...
import type { WebSearchProvider, WebSearchResult } from '@greeneek/gnk-web'
...
} from '@greeneek/gnk-tool-web'
import type { ContentBlock } from '@greeneek/gnk-llm'
import type { ToolResult } from '@greeneek/gnk-tools'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@greeneek/gnk-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by gnk-tool-call-timeout-policy over exec.signal.
