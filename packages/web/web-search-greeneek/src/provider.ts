/**
 * Greeneek search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @greeneek/gnk-web-search-greeneek/provider
 */
...
import type { CredentialRef } from '@greeneek/gnk-credentials'
...
export const GREENEEK_PROVIDER_ID = 'greeneek-official'

/**
 * Default endpoint: Greeneek's Anthropic-compatible API, `/v1` included
 * (`/messages` is appended). This is NOT the chat-completions base
 * (`https://api.greeneek.dev`) `@greeneek/gnk-llm-greeneek` uses, so this
 * provider does NOT reuse `$GREENEEK_BASE_URL` — only the API key is shared.
 */
export const GREENEEK_DEFAULT_BASE_URL = 'https://api.greeneek.dev/anthropic/v1'

/** Default Anthropic-format model name (aligned with the repo's Greeneek model vocabulary). */
export const GREENEEK_DEFAULT_MODEL = 'greeneek-v4-flash'

/** Default `anthropic-version` header value. */
export const GREENEEK_DEFAULT_API_VERSION = '2023-06-01'

/** Default upper bound on generated tokens for the Messages request. */
export const GREENEEK_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const GREENEEK_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'greeneek-harness/0.0.1'

/**
 * Exact secret-free Greeneek Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface GreeneekSearchLlmRequest {
...
declare module '@greeneek/gnk-session/types' {
  interface SessionEventMap {
    /** Secret-free auxiliary Greeneek search request recorded before dispatch. */
    'web/greeneek-search-llm-request': GreeneekSearchLlmRequest
  }
}

/** Resolved provider options (the plugin's `apply` supplies credential and constant defaults). */
export interface GreeneekSearchProviderOptions {
  /** Literal Greeneek API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current Greeneek API key for one search operation. */
   ...
   */
  recordRequest?: (request: GreeneekSearchLlmRequest) => void
}
...
/**
 * Map a Greeneek Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'Greeneek returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/**
 * The Greeneek-backed search provider. HTTP redirects fail as `WEB_PROVIDER_ERROR`;
 * failures after dispatch name the endpoint and tell the model how the user can configure it.
 */
export class GreeneekSearchProvider implements WebSearchProvider {
  readonly id = GREENEEK_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => GreeneekSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: GreeneekSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official Greeneek expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw searchEndpointError(
        endpoint,
        `Greeneek search request failed: ${String(error)}`,
        error,
      )
    }
    ...
    if (!response.ok) {
      ...
      let message = `Greeneek API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      const message = error instanceof WebError
        ? error.message
        : `Greeneek returned an unprocessable response body: ${String(error)}`
   ...
   */
  private async apiKey(options: GreeneekSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `Greeneek search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'GREENEEK_API_KEY'
    throw new WebError(
      `Greeneek search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-greeneek config',
...
function searchEndpointError(endpoint: string, message: string, cause?: unknown): WebError {
  return new WebError(
    ...
    + 'GREENEEK_SEARCH_BASE_URL or configure web-search-greeneek.baseURL to a trusted '
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('Greeneek search aborted', 'WEB_ABORTED', {
...
/** True for Greeneek request limits that can be sent to the Messages API. */
