...
import WebRuntime, { WebError } from '@greeneek/gnk-web'
import {
  GreeneekSearchProvider,
  GREENEEK_PROVIDER_ID,
} from '@greeneek/gnk-web-search-greeneek'
import * as greeneekPlugin from '@greeneek/gnk-web-search-greeneek'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@greeneek/gnk-web-search-greeneek/src/types.ts'
...
const options = {
  apiKey: 'ds-key',
  baseURL: 'https://api.greeneek.test/anthropic/v1',
  model: 'greeneek-chat',
...
describe('GreeneekSearchProvider availability', () => {
...
describe('GreeneekSearchProvider request mapping', () => {
...
describe('GreeneekSearchProvider settings changes mid-search', () => {
  it('serves one search from one section even when settings land during credential resolution', async () => {
    ...
    const provider = new GreeneekSearchProvider(() => ({ ...current, resolveApiKey }))
...
describe('GreeneekSearchProvider error handling', () => {
  ...
  it('maps a credential resolver rejection under an active signal to WEB_PROVIDER_ERROR', async () => {
    ...
    await expect(searchProvider({
      ...
      .rejects.toThrow(expect.objectContaining({
        ...
        message: 'Greeneek search credential resolution failed: Error: credential backend failed',
      }))
  })
  ...
  it('uses the default credential reference when no resolver is configured', async () => {
    await expect(searchProvider({ ...options, apiKey: '' }).search({ query: 'q' }))
      .rejects.toThrow('Greeneek search has no API key for "GREENEEK_API_KEY"')
  })
  ...
  it('maps an HTTP error to WEB_PROVIDER_ERROR with the provider message', async () => {
    ...
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({
        code: 'WEB_PROVIDER_ERROR',
        message: 'Greeneek API error (HTTP 429): rate limited\n\n'
          + 'The web search request used endpoint "https://api.greeneek.test/anthropic/v1/messages". '
  ...
  it('handles a string-form error body', async () => {
    ...
    expect(error.message).toContain('Greeneek API error (HTTP 400): bad request')
  })

  it('keeps a status-line message when the error body is not JSON', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response('upstream error', { status: 503 })))
    const error = await rejectedWebError(searchProvider(options).search({ query: 'q' }))
    expect(error.message).toContain('Greeneek API error (HTTP 503)')
  })

  it('keeps the status-line message when the JSON error body carries no detail', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({}, { status: 500 })))
    const error = await rejectedWebError(searchProvider(options).search({ query: 'q' }))
    expect(error.message).toContain('Greeneek API error (HTTP 500)')
  })
  ...
  it('maps a network failure to WEB_PROVIDER_ERROR', async () => {
    ...
    expect(error.message).toContain('The web search request used endpoint "https://api.greeneek.test/anthropic/v1/messages".')
  })
...
describe('web-search-greeneek plugin registration', () => {
  ...
  it('rejects maxTokens: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GREENEEK_PROVIDER_ID })
    await expect(ctx.plugin(greeneekPlugin, { apiKey: 'ds-key', maxTokens: 0 }))
  ...
  it('rejects maxUses: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GREENEEK_PROVIDER_ID })
    await expect(ctx.plugin(greeneekPlugin, { apiKey: 'ds-key', maxUses: 0 }))
  ...
  it('rejects a fractional maxUses at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GREENEEK_PROVIDER_ID })
    await expect(ctx.plugin(greeneekPlugin, { apiKey: 'ds-key', maxUses: 1.5 }))
      .rejects.toThrow(/maxUses expected number multiple of 1/)
  })

  it('has no default export (namespace plugin export shape)', () => {
    expect('default' in greeneekPlugin).toBe(false)
  })
  ...
  it('survives the real Loader unwrapExports path keeping name/inject/Config', () => {
    ...
    const unwrapped = loader.unwrapExports(greeneekPlugin) as Record<string, unknown>
    expect(unwrapped).toBe(greeneekPlugin)
    expect(unwrapped.name).toBe('web-search-greeneek')
  ...
  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    const previous = process.env.GREENEEK_API_KEY
    delete process.env.GREENEEK_API_KEY
    const dir = await mkdtemp(join(tmpdir(), 'gnk-web-search-credentials-'))
    ...
    try {
      ...
      if (previous === undefined) delete process.env.GREENEEK_API_KEY
      else process.env.GREENEEK_API_KEY = previous
    }
  })
