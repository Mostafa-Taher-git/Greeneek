...
Loading the engine registers `ctx.workflowEngine`; adding `gnk-tool-workflow` on top gives the model the `workflow` tool. Every config field is optional:

```yaml
- name: '@greeneek/gnk-workflow-worker-thread'
- name: '@greeneek/gnk-tool-workflow'
```
