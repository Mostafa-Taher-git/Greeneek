...
Trusted pull requests also run `--scenario sdk-live --installed-wheel` on every native target. That scenario performs two tool-using turns against the Greeneek-protocol endpoint named by `GREENEEK_BASE_URL` — this repository operates no such endpoint, so a keyed run must point at one the operator provides — verifies the created file externally, and self-skips when `GREENEEK_API_KEY` is absent instead of failing. Fork and Dependabot pull requests run the complete keyless installed-wheel path but receive no key.

An interactive smoke test needs `GREENEEK_API_KEY` in the environment or repository-root `.env`:

```python
from greeneek_harness import GreeneekHarness

with GreeneekHarness(gnk_home="/absolute/path/to/test-gnk-home") as harness:
    print(harness.run("say hi").final_response)
```

Alternatively export a non-empty `GNK_HOME`. The SDK rejects a launch that would silently use `~/.gnk`.

## Run against Node source

Repository contributors can select either development route; both execute the normal `gnk --profile sdk` launcher:

- Set `GNK_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
- Set `gnk_bin` to the absolute built `apps/cli/lib/bin.js` path to exercise the checkout's CLI directly. Supply an explicit `gnk_home`, plus `profile` and ordered `patches` as needed.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `greeneek-harness-runtime-bin` version.
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/greeneek-harness-sdk-runtime-macos-arm64 --output-dir dist-python
pip install \
  "dist-python/greeneek_harness_sdk-$version-py3-none-any.whl" \
  "dist-python/greeneek_harness_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
