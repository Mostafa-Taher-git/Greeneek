...
for (const pattern of markdownGlobs) {
  for (const match of globSync(pattern, { cwd: root })) {
    // Chinese sources are unowned legacy: only the English side carries checked contracts.
...
const useBuiltTypes = process.env.GNK_DOC_TYPECHECK_USE_BUILD_OUTPUT === '1'
