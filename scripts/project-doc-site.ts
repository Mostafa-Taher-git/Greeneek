...
import { docsPages, collections, orderedPages, type DocsPage } from '../website/docs.ts'
import {
...
const REPOSITORY_URL = 'https://github.com/greeneek/greeneek-harness'
...
function githubTarget(
  ...
  if (image) return `https://raw.githubusercontent.com/greeneek/greeneek-harness/${repositoryRef}/${path}${suffix}`
 ...
 *
 * The site links the repository from every page, so projecting the badge
 * would repeat it under each heading.
 *
 ...
 * page therefore keeps its body here, while the rendered site truncates
 ...
 * @returns llms.txt content listing the published tree.
 */
