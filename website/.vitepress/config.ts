 ...
 * the guide sidebar, shared by the navigation bar and the guide sidebar.
 */
const guideModules = {
  guide: 'guide',
...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/greeneek/greeneek-harness' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/greeneek/greeneek-harness/edit/master/${editSource}`
    },
...
const siteIdentity = {
  title: 'Greeneek Harness',
  description: 'Plugin SDK for building agent harnesses',
}

/**
 * The Greeneek wordmark, inlined so its `currentColor` fills follow the active
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="gnk-wordmark" ')
...
/**
 * Navigation-bar title: the Greeneek wordmark and the release-stage tag.
 * VitePress renders `siteTitle` as HTML.
 *
 * @returns Markup placed beside the navigation-bar home link.
 */
function siteTitle(): string {
  return `<span class="gnk-lockup">${wordmark}<span class="gnk-tag">Preview</span></span>`
}
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'greeneek-harness-doc-projector',
